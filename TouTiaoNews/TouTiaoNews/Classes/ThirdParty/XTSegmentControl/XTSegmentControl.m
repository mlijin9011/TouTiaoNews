...
#define XTSegmentControlHspace (10)

#define XTSegmentControlLineHeight (26)
...
- (void)addRedLine
{
    if (!_lineView) {
        ...
        _lineView = [[UIView alloc] initWithFrame:CGRectMake(XTSegmentControlHspace/2, (CGRectGetHeight(rect) - XTSegmentControlLineHeight)/2, CGRectGetWidth(rect) -  XTSegmentControlHspace, XTSegmentControlLineHeight)];
        _lineView.backgroundColor = [UIColor colorWithRed:0.776471 green:0.196078 blue:0.207843 alpha:0.9];
        _lineView.layer.cornerRadius = 8;
        [_contentView insertSubview:_lineView atIndex:0];
...
- (void)selectIndex:(NSInteger)index
{
    ...
    if (index != _currentIndex) {
        ...
        CGRect lineRect = CGRectMake(CGRectGetMinX(rect) + XTSegmentControlHspace/2, (CGRectGetHeight(rect) - XTSegmentControlLineHeight)/2, CGRectGetWidth(rect) - XTSegmentControlHspace, XTSegmentControlLineHeight);
        [UIView animateWithDuration:XTSegmentControlAnimationTime animations:^{
            _lineView.frame = lineRect;
        }];
        
//        [self setSelectTitleColor:index];
...
- (void)setSelectTitleColor:(NSInteger)index
{
    ((XTSegmentControlItem *)[_contentView viewWithTag:(1000 + index)]).titleLabel.textColor = [UIColor whiteColor];
    ((XTSegmentControlItem *)[_contentView viewWithTag:(1000 + _currentIndex)]).titleLabel.textColor = [UIColor darkGrayColor];
    
    [self layoutSubviews];
}

- (void)moveIndexWithProgress:(double)progress
{
    double delta = progress - _currentIndex;

    CGRect origionRect = [_itemFrames[_currentIndex] CGRectValue];;
    
    CGRect origionLineRect = CGRectMake(CGRectGetMinX(origionRect) + XTSegmentControlHspace/2, (CGRectGetHeight(origionRect) - XTSegmentControlLineHeight)/2, CGRectGetWidth(origionRect) - XTSegmentControlHspace, XTSegmentControlLineHeight);
